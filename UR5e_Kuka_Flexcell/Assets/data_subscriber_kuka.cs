...
public class data_subscriber_kuka : MonoBehaviour
{
    ...
    void Listener(){
        try
        {
            using (var subSocket = new SubscriberSocket()){
                ...
                //subSocket.Connect("tcp://192.168.1.151:5557");
                Debug.Log("Kuka subscriber socket connecting...");
    ...
    {
        if (firstTime == true){
            UpdateJointTheta(1, 0.0f);
            UpdateJointTheta(2, 0.0f);
            UpdateJointTheta(3, 0.0f);
            UpdateJointTheta(4, 0.0f);
            UpdateJointTheta(5, 0.0f);
            UpdateJointTheta(6, 0.0f);
            UpdateJointTheta(7, 0.0f);
            firstTime = false;
        }
